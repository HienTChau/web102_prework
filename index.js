...
function addGamesToPage(games) {
    for (let game of games) { // loop over each item in the data
...
let displayStr = `A total of $${totalRaised.toLocaleString()} has been rasied for ${GAMES_JSON.length} games. ${countUnfunded > 1 ? `Currently, ${countUnfunded} games remain unfunded.` : `Currently, ${countUnfunded} game remains unfunded.`}`;
displayStr += ` We need your help to fund these amazing games!`;
...
// Add an event listener to the search input to filter games based on user input
